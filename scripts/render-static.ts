...
const parseSvg = async (url: string) => {
  ...
  const label = $('svg > g:last-child > text:first-child').text() || $('svg > g:nth-child(4) > text:first-child').text()
  const value = $('svg > g:last-child > text:last-child').text() || $('svg > g:nth-child(4) > text:last-child').text()
  return {
    label,
    value,
  };
...
async function job(list: string[]) {
  ...
  await writeFile(dest, JSON.stringify(result.reduce((acc, item) => ({ ...acc, [item.label]: item }), {})))
}
